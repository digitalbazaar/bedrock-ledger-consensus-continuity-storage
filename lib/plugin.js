  ...
  api: {
    expandIndexes: async ({createIndexes, collections}) => {
      await createIndexes([{
        ...
        // this index is used to prevent reuse of parents on merge events
        // by a creator
        collection: collections.eventCollection,
        fields: {
          'meta.continuity2017.creator': 1,
          // FIXME: add `meta.fork` identifier as well, we must allow for
          // forks to reuse parents, but non-forks must not
          'event.parentHash': 1
        },
        options: {
          unique: true, background: false,
          name: 'event.continuity2017.parent',
          // only merge events have creator/this index only applies to merges
          partialFilterExpression: {'meta.continuity2017.type': 'm'}
        }
      }, {
        // this index is exclusively for events.outstandingRegularEvent
        // FIXME: potentially also used by non-consensus peer head lookup,
        // if so, index needs a rename
        collection: collections.eventCollection,
        fields: {
          'meta.consensus': 1,
          'meta.continuity2017.type': 1,
